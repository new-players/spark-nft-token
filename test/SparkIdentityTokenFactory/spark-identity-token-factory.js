...
describe("Spark Identity Token Factory", async () => {
    beforeEach(async () => {
        ...
        const SparkIdentityTokenFactoryFactory = await ethers.getContractFactory(
    ...
    it("Factory Deployment with zero address should fail", async () => {
        ...
        await expect(
            SparkIdentityTokenFactoryFactory.deploy(ethers.ZeroAddress)
        ).to.be.revertedWithCustomError(SparkIdentityTokenFactoryFactory, "ZeroAddressNotAllowed");
    });
