...
describe("ERC6551 Manager Unit Test Cases", () => {
    beforeEach(async () => {
        ...
        const exisitingConfig = JSON.parse(
            await fs.readFile("config/deployment-config.json", "utf8")
        );

        const { registry, tokenboundAccountProxy, tokenboundAccountImplementation, salt } =
            exisitingConfig[network.name === "hardhat" ? "local" : network.name].ERC6551Manager;
        ...
        const erc6551ManagerTx = await ERC6551ManagerFactory.deploy(
            ERC6551Registry,
            ERC6551AccountProxy,
    ...
    it("Check a non admin user cannot grant or revoke roles", async () => {
        ...
        await expect(
            ERC6551Manager.connect(userOne).grantRole(adminRole, userOne.address)
        ).to.be.revertedWithCustomError(ERC6551Manager, "AccessControlUnauthorizedAccount");
        await expect(
            ERC6551Manager.connect(userOne).revokeRole(adminRole, userOne.address)
        ).to.be.revertedWithCustomError(ERC6551Manager, "AccessControlUnauthorizedAccount");
    });
    ...
    it("Check a new admin can grant or revoke another admin roles", async () => {
        ...
        await expect(ERC6551Manager.grantRole(adminRole, userOne.address)).not.to.be.reverted;
        await expect(await ERC6551Manager.hasRole(adminRole, userOne.address)).to.equal(true);
        await expect(ERC6551Manager.connect(userOne).grantRole(adminRole, userTwo.address)).not.to
            .be.reverted;
        await expect(ERC6551Manager.connect(userOne).revokeRole(adminRole, userTwo.address)).not.to
            .be.reverted;
    });
    ...
    it("Configure ERC6551 registry address", async () => {
        await expect(
            ERC6551Manager.setupERC6551Registry(ethers.ZeroAddress)
        ).to.be.revertedWithCustomError(ERC6551Manager, "ZeroAddressNotAllowed");
    ...
    it("Configure ERC6551 Proxy address", async () => {
        await expect(
            ERC6551Manager.setupERC6551Proxy(ethers.ZeroAddress)
        ).to.be.revertedWithCustomError(ERC6551Manager, "ZeroAddressNotAllowed");
    ...
    it("Configure ERC6551 implementation address", async () => {
        await expect(
            ERC6551Manager.setupERC6551Implementation(ethers.ZeroAddress)
        ).to.be.revertedWithCustomError(ERC6551Manager, "ZeroAddressNotAllowed");
    ...
    it("Configure ERC6551 salt", async () => {
        const abiCoder = new ethers.AbiCoder();
        await expect(ERC6551Manager.setupERC6551Salt(abiCoder.encode(["uint256"], [1]))).not.to.be
            .reverted;
        await expect(await ERC6551Manager.erc6551Salt()).to.equal(
