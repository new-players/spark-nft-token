...
module.exports = async ({ deployments }) => {
    ...
    const SparkRegistry = await ethers.getContractAt(
        "SparkRegistry",
        SparkRegistryInfo.contractAddress,
        deployer
    );
    const SparkIdentity = await ethers.getContractAt(
        "SparkIdentity",
    ...
    const grantMinterRoleTx = await SparkIdentity.grantRole(
        minterRole,
        SparkRegistryInfo.contractAddress
    );
    ...
    if (ERC6551ManagerInfo.contractAddress && isAddress(ERC6551ManagerInfo.contractAddress)) {
        const erc6551ManagerTx = await SparkRegistry.configureERC6551Manager(
            ERC6551ManagerInfo.contractAddress
        );
        await erc6551ManagerTx.wait(waitConfirmation);

        log(
            "Spark Registry (ERC6551 manager) is configured to: ",
            ERC6551ManagerInfo.contractAddress
        );
    }

    if (SparkRegistryInfo.isPaymentEnabled) {
        const nativePaymentAmountInWei = ethers.parseEther(
            SparkRegistryInfo.nativePaymentAmountInEther
        );

        if (nativePaymentAmountInWei > 0) {
            const nativePaymentTx = await SparkRegistry.configurePayment(
                SparkRegistryInfo.beneficiaryAddress,
                SparkRegistryInfo.isPaymentEnabled,
                nativePaymentAmountInWei
            );
            await nativePaymentTx.wait(waitConfirmation);

            log(
                "Spark Registry payment is configured with these options: ",
                JSON.stringify({
        ...
        if (SparkRegistryInfo.paymentTokens.length > 0) {
            const filteredTokenInfo = SparkRegistryInfo.paymentTokens.filter(
                (token) => ethers.parseEther(token.amountInEther) > 0
            );
            ...
            log(
                "Spark Registry payment tokens are configured with these options: ",
                JSON.stringify({
    ...
    if (SparkRegistryInfo.isRewardsEnabled) {
        const maxRewardsPerUserInWei = ethers.parseEther(
            SparkRegistryInfo.maxRewardsPerUserInEther
        );
        ...
        if (maxRewardsPerUserInWei > 0 && rewardsPerMintInWei > 0) {
            if (rewardsPerMintInWei <= maxRewardsPerUserInWei) {
                const rewardConfigTx = await SparkRegistry.configureRewards(
                    rewardsPerMintInWei,
                    maxRewardsPerUserInWei,
                    SparkRegistryInfo.isRewardsEnabled
                );
                await rewardConfigTx.wait(waitConfirmation);

                log(
                    "Spark Registry rewards are configured with these options: ",
                    JSON.stringify({
        ...
        if (SparkRegistryInfo.rewardableNfts.length > 0) {
            ...
            log(
                "Spark Registry rewardable nfts are configured with these options: ",
                JSON.stringify({
